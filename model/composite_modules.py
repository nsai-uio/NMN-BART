...
class AndModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in):
        ...
        _att_stack = _write_to_stack(att_stack, stack_ptr, torch.zeros(feat.size(0),feat.size(1),1).to(feat.device))
        stack_ptr_out = _move_ptr_bw(stack_ptr)
        att1 = _read_from_stack(_att_stack, stack_ptr_out)
        att_out = torch.min(att1, att2)
        att_stack_out = _write_to_stack(_att_stack, stack_ptr_out, att_out)
        mem_in_out = mem_in.clone().zero_()
        return att_stack_out, stack_ptr_out, mem_in_out 
...
class FindModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in):
        ...
        _att_out = self.x_conv(self.drop(x)) # (batch_size, num_feat, glimpse)
        att_out = F.softmax(_att_out, dim=1)  # (batch_size, num_feat, glimpse)
        #att_out = torch.sigmoid(att_out)
        stack_ptr_out = _move_ptr_fw(stack_ptr)
        att_stack_out = _write_to_stack(att_stack, stack_ptr_out, att_out)
        mem_in_out = mem_in.clone().zero_()
        return att_stack_out, stack_ptr_out, mem_in_out
...
class FilterModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in):
        _att_stack, _stack_ptr, _ = self.Find(vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in)
        att_stack, stack_ptr, _ = self.And(vision_feat, feat, feat_edge, c_i, relation_mask, _att_stack, _stack_ptr, mem_in)
        mem_in_out = mem_in.clone().zero_()
        return att_stack, stack_ptr, mem_in_out
...
class TransformModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in):
        ...
        _weit_matrix = F.sigmoid(torch.sum(elt_prod, dim=3))
        weit_matrix = _weit_matrix * relation_mask.float()
        att_in = _read_from_stack(att_stack, stack_ptr).permute(0,2,1) #(batch_size, glimpse, att_dim)
        _att_out = torch.matmul(att_in, weit_matrix).permute(0,2,1) #(batch_size, att_dim, glimpse)
        norm = torch.max(_att_out, dim=1, keepdim=True)[0].detach()
        norm[norm<=1] = 1
        # att_out /= norm
        att_out = _att_out/norm
        # ---------
        att_stack_out = _write_to_stack(att_stack, stack_ptr, att_out)
        mem_in_out = mem_in.clone().zero_()
        return att_stack_out, stack_ptr, mem_in_out
...
class DescribeModule(nn.Module):
    ...
    def forward(self, vision_feat, feat, feat_edge, c_i, relation_mask, att_stack, stack_ptr, mem_in):
        ...
        _mem_out = torch.bmm(att_in, vision_feat)

        mem_out = _mem_out.view(batch_size, -1) #(batch_size, glimpse*dim_vision)
...
def _move_ptr_fw(stack_ptr):
    ...
    out_stack_ptr = new_stack_ptr + stack_top_mask * new_stack_ptr
    return out_stack_ptr
...
def _move_ptr_bw(stack_ptr):
    ...
    out_stack_ptr = new_stack_ptr + stack_bottom_mask * new_stack_ptr
    return out_stack_ptr
